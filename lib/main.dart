    ...
    MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => NotificationsBloc(
          requestPermissionsLocalNotifications: LocalNotifications.requestPermissionsLocalNotifications,
...
class _HandleNotificationInteractionsState extends State<HandleNotificationInteractions> {
  ...
  void _handleMessage(RemoteMessage message) {
    // Add remoteMessage to notifications bloc
    context.read<NotificationsBloc>().handleRemoteMessage(message);

    // Format message id
    String messageId = message.messageId?.replaceAll(':', '').replaceAll('%', '') ?? '';

    // Navigate to details sending message id as param
